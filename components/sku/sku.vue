<!-- 此sku组件已经运用了vuex，若要使用，需要将vuex删除 -->
			...
			<view class="layer attr-content">
				<view class="specification-wrapper">
					<scroll-view class="specification-wrapper-content" scroll-y="true">
						<view class="specification-header">
							<view class="specification-left">
								<image @tap="onPreviewImage" :data-src="shopItem.img" class="product-img"
									:src="shopItem.img"></image>
							</view>
							...
							<view class="specification-item" v-for="(item, index1) in specifications" :key="index1">
								...
								<view class="item-wrapper">
									<view class="item-content"
										@tap="skuClick(item_value[item.key], index1, item_value, index2,item.key)"
										v-for="(item_value, index2) in item.item" :key="index2"
										:class="[item.activeIndex == index2 ? 'actived' : '']">
										{{ item_value[item.key]}}
									</view>
								</view>
							</view>
							<!-- 因为sku组件的data和购物车的data中选中number数不一样，所以暂不用此view -->
							<!-- <view class="specification-item specification-item_display">
								<view class="item-title">数量</view>
								<view class="item-wrapper">
									<stepper size="small" :min="goodMinNum" :max="goodMaxNum" @change="changeNum">
									</stepper>
								</view>
							</view> -->
						</view>
					...
					<view class="close" @tap="hideSKU">
						<u-icon class="close-item" size="40" name="close"></u-icon>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			onPreviewImage(e) {
				console.log(e)
				var dataset = e.currentTarget.dataset.src;
				// 预览图片  goodImgList
				uni.previewImage({
					urls: [dataset],
					longPressActions: {
			...
			changeNum(val) {
				...
				// 将数量传给vuex
				// this.$store.state.goodsMessage[i] = parseInt(val)
			},
