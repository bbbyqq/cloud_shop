...
<!-- 此购物车组件在选择商品方面有一定问题，勾选对应的商品，返回的是全部值，删除功能也未实现，故谨慎使用 -->
	...
	<template>
		<view class="common-car">
			<view class="empty-shop-car" v-if="isEmpty">
				...
				<view class="empty-shop-car-btn" @click="goToIndex">
					<text style="color: #FFFFFF;">去逛逛</text>
				</view>
			</view>
			<view class="shop-car" v-else>
				...
				<view class="store-box" v-for="(itemq,indexq) in datas" :key="indexq">
					...
					<image src="../../static/images/cart/select.png" v-if="itemq.checked == 2" class="checked-image" mode="" @tap="storeCheck(indexq,itemq.checked)"></image>
					<image src="../../static/images/cart/not_select.png" v-else class="checked-image" mode="" @tap="storeCheck(indexq,itemq.checked)">
					</image>
					<text>{{itemq.name}}</text>
					<image src="../../static/images/cart/right.png" class="label" mode=""></image>
				</view> -->
					<view class="goodsInfo" v-for="(itemw,indexw) in itemq.goods" :key="indexw">
						<image src="../../static/images/cart/select.png" v-if="itemw.checked == 2" class="checked-image"
							mode="" @tap="goodsCheck(indexq,indexw,itemw.checked)"></image>
						<image src="../../static/images/cart/not_select.png" v-else class="checked-image" mode=""
							@tap="goodsCheck(indexq,indexw,itemw.checked)"></image>
						<view class="goodsInfo-right">
							<image :src="itemw.img" class="goods-image" mode="" @click="goToGoodsDetail"></image>
							<view class="goodsInfo-box">
								<text class="goods-name">{{itemw.title}}</text>
								<text class="spe">规格：{{itemw.name_first}} {{itemw.name_second}} {{itemw.name_third}}</text>
				...
				<view class="statistics-box">
					<view class="statistics">
						<view class="statistics-left" v-if="statisticsIndex" @tap="allCheck">
							<image src="../../static/images/cart/select.png" class="checked-image" mode="">
							</image>
							<text>全选</text>
						</view>
						<view class="statistics-left" v-else @tap="allCheck">
							<image src="../../static/images/cart/not_select.png" class="checked-image" mode="">
							</image>
							<text>全选</text>
						</view>
		...
		export default {
			...
			methods: {
				...
				goodsCheck(storeIndex, goodsIndex, goodsChecked) {
					if (goodsChecked == 1) {
						this.goods.push(this.datas[storeIndex].goods[goodsIndex])
						this.datas[storeIndex].goods[goodsIndex].checked = 2
					} else {
						this.goods.splice(goodsIndex, goodsIndex + 1)
				...
				//全选
				allCheck() {
					this.goods.push(this.datas[0].goods[0])
